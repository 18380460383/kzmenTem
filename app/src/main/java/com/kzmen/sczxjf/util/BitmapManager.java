...
public class BitmapManager {
    ...
    public void loadBitmap(String url, ImageView imageView) {  
        loadBitmap(url, imageView, this.defaultBmp, 100, 100);
    }
    ...
    public void loadBitmap(String url, ImageView imageView, Bitmap defaultBmp) {  
        loadBitmap(url, imageView, defaultBmp, 100, 100);
    }
    ...
    public void loadBitmap(String url, ImageView imageView, Bitmap defaultBmp, int width, int height) {  
        ...
        if (bitmap != null) {  
            ...
            TLog.log("cache  load bitmap: " + url);
            ...
            if(file.exists() && !file.isDirectory()){
                ...
                Log.i("tag", cache.size() + "c");
                if(cache.size()>3){
                    Set<String> strings = cache.keySet();
                    for(int i=0;i<1;i++){
                        cache.remove(strings.iterator().next());
                    }
                }
                Log.i("tag", cache.size() + "d");
            }else{
                //线程加载网络图片
                TLog.log("net  load bitmap: " + url);
            ...
            public void handleMessage(Message msg) {  
                ...
                if (tag != null && tag.equals(url)) {  
                    if (msg.obj != null) {  
                        ...
                        try {
                            //向SD卡中写入图片缓存
                            ImageUtils.saveImage(imageView.getContext(), FileUtil.getFileName(url), (Bitmap) msg.obj);
                        } catch (IOException e) {
                            e.printStackTrace();
                        }
                    } 
